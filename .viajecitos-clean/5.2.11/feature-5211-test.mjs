...
await import(`${new URL('./feature-5211.mjs', import.meta.url).href}?test=${Date.now()}`);
await new Promise((resolve) => setTimeout(resolve, 20));

const assert = (condition, message) => {
  if (!condition) throw new Error(`VJT_5211_TEST_FAILED: ${message}`);
};

const spotify = document.querySelector('.vjt-platform-spotify');
assert(spotify.dataset.vjtSpotifyPolished === '5.2.11-spotify-members', 'Spotify no se ha actualizado.');
...
console.log('VJT_5211_EXTENSION_TESTS_OK', JSON.stringify({
